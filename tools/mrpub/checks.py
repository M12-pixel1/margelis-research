...
class NoteChecks:
    ...
    def run(self, built: bool = True, only: str | None = None) -> list[Result]:
        ...
        for name in names:
            ...
            if only is not None and only not in cid:
                continue
            try:
    ...
    def c_markdown_structure(self):
        ...
        present = {strip_number(h) for h in h2}
    ...
    def c_external_links(self):
        ...
        if transient:
            return WARN, f"{len(urls) - len(transient)} URLs OK; unreachable right now (retry later): {transient}"
        return PASS, f"{len(urls)} external URLs return HTTP 200"
    ...
    def c_license_consistency(self):
        ...
        if n.license_granted:
            ...
            lic_section = readme.split("## License", 1)[1] if "## License" in readme else readme
            if spdx not in lic_section or re.search(r"(?im)^\s*pending\.?\s*$", lic_section):
                problems.append("README license section does not state the granted license")
        else:
...
def check_benchmark() -> list[Result]:
    ...
    out.append(check_benchmark_docs_consistency(root))
    return out


def check_docs_current() -> Result:
    """README files must describe the current version of every note (they are regenerated by build)."""
    from .build import readme_notes_block, research_readme
    problems = []
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    notes = [load_note(n) for n in all_notes()]
    start, end = "<!-- notes:start -->", "<!-- notes:end -->"
    if start not in readme or end not in readme:
        problems.append("README.md lacks the generated notes block markers")
    else:
        current = readme.split(start, 1)[1].split(end, 1)[0]
        if current.strip() != readme_notes_block(notes).strip():
    ...
    for n in notes:
        ...
        wrong = sorted({v for v in stale if v != n.version and v not in n.previous_versions()})
        if wrong:
            problems.append(f"README.md refers to unknown release versions {wrong} of note {n.number}")
...
def check_dependency_lock() -> Result:
    ...
    return Result("repo.dependency_lock", PASS if ok else FAIL, detail)


def check_benchmark_docs_consistency(root: Path) -> Result:
    """Benchmark documents must agree with results/ about whether any result bundle is published."""
    results = root / "results"
    bundles = sorted(p.name for p in results.iterdir() if p.is_dir()) if results.exists() else []
    docs = [root / "README.md", root / "SPEC.md", root / "live" / "README.md", root / "harness" / "README.md",
            root / "sandbox" / "README.md", ROOT / "README.md"]
    denials = [r"results/\s+stays\s+empty", r"results/\s*\(empty\)", r"empty until", r"no public benchmark results",
               r"no (?:implementation and no )?results are published", r"no benchmark results are published"]
    problems = []
    if bundles:
        for doc in docs:
            if not doc.exists():
                continue
            text = norm(doc.read_text(encoding="utf-8")).lower()
            for pat in denials:
                m = re.search(pat, text)
                if m:
                    problems.append(f"{doc.relative_to(ROOT).as_posix()} says '{m.group(0)}' although results/ holds {bundles}")
        index = (results / "README.md").read_text(encoding="utf-8") if (results / "README.md").exists() else ""
        for b in bundles:
            if f"{b}/" not in index:
                problems.append(f"results/README.md does not list bundle {b}")
    else:
        index = (results / "README.md").read_text(encoding="utf-8") if (results / "README.md").exists() else ""
        if re.search(r"\]\([a-z0-9\-]+/\)", index):
            problems.append("results/README.md lists a bundle although results/ has none")
    return Result("benchmark.docs_consistency", FAIL if problems else PASS,
                  "; ".join(problems) if problems else
                  (f"documents agree: {len(bundles)} published bundle(s), all listed in results/README.md"
                   if bundles else "documents agree: no published bundle"))
